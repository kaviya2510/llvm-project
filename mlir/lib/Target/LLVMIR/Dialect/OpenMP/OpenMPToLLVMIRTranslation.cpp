...
convertOmpTaskloopOp(Operation &opInst, llvm::IRBuilderBase &builder,
  ...
  Region &loopRegion = taskloopOp.getRegion();

  // for (auto &block : loopRegion.getBlocks()) {
  ...
  // Set up inserytion point for call to createTaskloop()
  ...
  auto LoopGenCB = [&](llvm::OpenMPIRBuilder::InsertPointTy ip,
                       llvm::Value *iv) -> llvm::Error {
    llvm::outs() << "LoopGenCB called for taskloop\n";
    for (auto &block : loopRegion.getBlocks()) {
      block.print(llvm::outs());
    }

    moduleTranslation.mapValue(loopRegion.front().getArgument(0), iv);

    builder.restoreIP(ip);

    llvm::Expected<llvm::BasicBlock *> regionBlock = convertOmpOpRegions(
        loopRegion, "omp.loop_nest.region", builder, moduleTranslation);
    if (!regionBlock)
  ...
  llvm::OpenMPIRBuilder::InsertPointOrErrorTy afterIP =
      moduleTranslation.getOpenMPBuilder()->createTaskloop(
          ompLoc, allocaIP, bodyCB, LoopGenCB, loopInfo,
